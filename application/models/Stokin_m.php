...
class Stokin_m extends CI_Model
{
    ...
    private function _get_data_TransaksiListIncoming()
    {
        if ($_POST['id_transaksi'] != null) {
            $this->db->select('i.item_code AS kode_brg, i.name AS nama_brg, td.qty, s.name AS satuan_name, td.price, td.total_price, td.id_transaksi_d, td.batch, td.expired');
    ...
    public function create($post)
    {
        ...
        $id_transaksi_d = $this->db->insert_id();
        $object = [
            'reff' => $id_transaksi_d
        ];
        $this->db->where('id_transaksi_d', $id_transaksi_d);
        $this->db->update('transaksi_d', $object);
